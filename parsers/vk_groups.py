...
# Целевая география: Крым + Запорожская обл. + Херсонская обл.
# Используется для гео-фильтра (substring по нижнему регистру).
TARGET_MARKERS = (
    # регионы
    "крым", "запорожск", "херсонск", "таврия", "таврическ",
    # Крым: города
    "симферополь", "севастополь", "ялта", "евпатория", "феодосия",
    "керчь", "алушта", "судак", "саки", "бахчисарай", "джанкой",
    "армянск", "красноперекопск", "белогорск", "старый крым",
    "щёлкино", "щелкино", "черноморское",
    # Крым: районы (substring покрывает «-ий район»)
    "симферопольск", "бахчисарайск", "джанкойск", "кировск",
    "красногвардейск", "ленинск", "нижнегорск", "первомайск",
    "раздольненск", "сакск", "советск", "черноморск",
    # Крым: посёлки
    "первомайское", "красногвардейское", "октябрьское", "ленино",
    "нижнегорский", "раздольное", "советский", "кировское",
    "гвардейское", "мирное", "молодёжное", "молодежное",
    "николаевка", "заозёрное", "заозерное", "гаспра", "гурзуф",
    "массандра", "ливадия", "форос", "симеиз", "алупка", "партенит",
    "коктебель", "орджоникидзе", "приморский", "новофёдоровка",
    "новофедоровка", "балаклава", "морское", "малореченское",
    "береговое", "кастрополь", "мисхор", "зуя", "межводное",
    "мысовое", "береговое", "оленевка",
    # Запорожская область
    "мелитополь", "бердянск", "приазовское", "днепрорудный",
    "днепрорудное", "энергодар", "каменка-днепровская", "токмак",
    "акимовка", "весёлое", "веселое", "михайловка", "васильевка",
    "пологи", "куйбышево", "черниговка",
    # Херсонская область
    "геническ", "новотроицкое", "новоалексеевка", "каланчак",
    "чаплынка", "скадовск", "белозерка", "белозёрка",
    "голая пристань", "алёшки", "алешки", "каховка", "новая каховка",
    "таврийск", "великая лепетиха", "верхний рогачик", "ивановка",
    "нижние серогозы",
)

# Бэкомпат: оставляем старое имя для внешнего кода (если где-то импортируется)
CRIMEA_MARKERS = TARGET_MARKERS


def _is_target_region(city: str, name: str, description: str) -> bool:
    """True, если запись относится к Крыму / Запорожской / Херсонской обл.

    Главное правило: если city ЯВНО указан (любой), мы доверяем ему — не лезем
    в text-fallback. Иначе пропускали бы группы типа «Бюро экскурсий по
    Запорожскому краю» с city=«Запорожье» только потому, что в name есть
    «запорожск». Text-fallback включается только когда city не определён
    (пусто или fallback «Крым» из found.get(gid, «Крым»)).
    """
    low_city = (city or "").strip().lower()
    if low_city:
        # есть явный город — он либо в наших маркерах, либо чужой; точка.
        # «Запорожск» как substring не помещается в «запорожье»/«херсон» →
        # сами эти регцентры отсекаются автоматически.
        return any(m in low_city for m in TARGET_MARKERS)
    # city пустой — пробуем подтвердить регион по тексту
    text = (name + " " + description).lower()
    return any(m in text for m in ("крым", "запорожск", "херсонск", "таврия"))


# Бэкомпат для прежнего имени
_is_crimea = _is_target_region


# Региональные точки для city-less VK-запросов
# (для каждой делаем «кондиционеры {X}» и «установка кондиционеров {X}»)
REGIONAL_POINTS = (
    # Крым доп.
    "Джанкой", "Армянск", "Красноперекопск", "Белогорск", "Старый Крым",
    "Щёлкино", "Черноморское", "Николаевка",
    # Запорожская обл.
    "Мелитополь", "Бердянск", "Энергодар", "Днепрорудный",
    "Каменка-Днепровская", "Токмак", "Васильевка", "Михайловка",
    "Приморск", "Пологи", "Акимовка",
    # Херсонская обл.
    "Геническ", "Скадовск", "Каховка", "Новая Каховка",
    "Голая Пристань", "Алёшки", "Таврийск", "Новотроицкое",
    "Каланчак", "Чаплынка",
)
...
# Ключевые слова для фильтрации релевантных HVAC-групп.
# Намеренно строгие — НЕ включаем общие «монтаж»/«установка»/«кондиц»,
# иначе пропускаются туризм/перевозки/бурение скважин (у них в описаниях
# тоже есть слово «установка»/«монтаж»).
RELEVANCE_KEYWORDS = (
    "кондиционер", "сплит-систем", "сплит систем",
    "климат", "вентил", "hvac", "холодильн", "чиллер", "фреон",
)
...
async def run(context):
    ...
    # Дополнительный поиск без привязки к VK city_id —
    # для регионов и городов, у которых нет своего VK city_id.
    regional_queries = [
        "кондиционеры Крым", "установка кондиционеров Крым", "сплит Крым",
    ...
    for pt in REGIONAL_POINTS:
        regional_queries.append(f"кондиционеры {pt}")
        regional_queries.append(f"установка кондиционеров {pt}")

    for q in regional_queries:
        r = _call("groups.search", token, q=q, count=200, sort=0)
        time.sleep(RPS_PAUSE)
        # Привязка fallback-города: для регион-запросов используем имя региона,
        # чтобы потом гео-фильтр и человекочитаемые отчёты не сводили всё в «Крым».
        q_low = q.lower()
        if "запорожск" in q_low:
            fallback_city = "Запорожская обл."
        elif "херсонск" in q_low or "таврия" in q_low:
            fallback_city = "Херсонская обл."
        elif "крым" in q_low:
            fallback_city = "Крым"
        else:
            fallback_city = q.split()[-1]
        for it in r.get("response", {}).get("items", []):
            gid = it.get("id")
            if gid and gid not in found:
                found[gid] = fallback_city
    print(f" [VK] +{len(regional_queries)} региональных запросов")
