...
async def run(context):
    ...
    for q in regional_queries:
        ...
        for it in r.get("response", {}).get("items", []):
            gid = it.get("id")
            if gid and gid not in found:
                # Город «привязываем» к названию точки из запроса,
                # но финальное решение принимает _is_target_region по тексту группы.
                found[gid] = q.split()[-1] if "област" not in q else "Крым"
